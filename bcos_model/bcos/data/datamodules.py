...
from .categories import CIFAR10_CATEGORIES, IMAGENET_CATEGORIES, CELEBA_CLASSES, WATERBIRDS_CLASSES
from .sampler import RASampler
from .transforms import RandomCutmix, RandomMixup, SplitAndGrid

__all__ = ["ImageNetDataModule", "CIFAR10DataModule", "ClassificationDataModule", "waterbirdsDataModule","celebADataModule"]
...
class waterbirdsDataModule(ClassificationDataModule):
    NUM_CLASSES: int = len(WATERBIRDS_CLASSES)

    NUM_TRAIN_EXAMPLES: int = 4795
    NUM_EVAL_EXAMPLES: int = 5794
    CATEGORIES: List[str] = WATERBIRDS_CLASSES
    ...
    def setup(self, stage: str) -> None:
        ...
        WATERBIRDS_PATH = settings.WATERBIRDS_PATH
        if stage == "fit":
            cache_dataset = self.config.get("cache_dataset", None)
            rank_zero_info("Setting up WaterBirds train dataset...")
            start = time.perf_counter()
            train_root = os.path.join(
                SHMTMPDIR if cache_dataset == "shm" else WATERBIRDS_PATH,
                "train",
            )
        ...
        start = time.perf_counter()
        rank_zero_info("Setting up WaterBirds val dataset...")
        self.eval_dataset = ImageFolder(
            root=os.path.join(WATERBIRDS_PATH, "test"),
            transform=self.config["test_transform"],
        )
        assert len(self.eval_dataset) == self.NUM_EVAL_EXAMPLES
        rank_zero_info(f"Done! Took time {time.perf_counter() - start:.2f}s")

class celebADataModule(ClassificationDataModule):
    NUM_CLASSES: int = 2

    NUM_TRAIN_EXAMPLES: int = 162_770
    NUM_EVAL_EXAMPLES: int = 19_962

    CATEGORIES: List[str] = CELEBA_CLASSES
    ...
    def setup(self, stage: str) -> None:
        # this way changes to the settings are reflected at function call time
        SHMTMPDIR = settings.SHMTMPDIR
        CELEBA_PATH = settings.CELEBA_PATH
        if stage == "fit":
            cache_dataset = self.config.get("cache_dataset", None)
            rank_zero_info("Setting up CelebA train dataset...")
            start = time.perf_counter()
            train_root = os.path.join(
                SHMTMPDIR if cache_dataset == "shm" else CELEBA_PATH,
                "train",
            )
            self.train_dataset = ImageFolder(
                root=train_root,
        ...
        start = time.perf_counter()
        rank_zero_info("Setting up CelebA val dataset...")
        self.eval_dataset = ImageFolder(
            root=os.path.join(CELEBA_PATH, "test"),
            transform=self.config["test_transform"],
        )
        assert len(self.eval_dataset) == self.NUM_EVAL_EXAMPLES
        rank_zero_info(f"Done! Took time {time.perf_counter() - start:.2f}s")
